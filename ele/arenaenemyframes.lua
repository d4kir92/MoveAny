...
function MoveAny:InitArenaEnemyFrames()
	if MoveAny:IsEnabled("ARENAENEMYFRAMES", false) and Arena_LoadUI then
		...
		MAArenaEnemyFrames = CreateFrame("FRAME", "MAArenaEnemyFrames", MABack)
		MAArenaEnemyFrames:SetSize(ArenaEnemyFrames:GetSize())
		MAArenaEnemyFrames:SetPoint("TOPRIGHT", MABack, "TOPRIGHT", 100, 100)
		...
		hooksecurefunc(MAArenaEnemyFrames, "SetAlpha", function(sel, alpha)
			ArenaEnemyFrames:SetAlpha(alpha)
		end)

		if MoveAny:DEBUG() then
			hooksecurefunc(ArenaEnemyFrames, "Hide", function(sel)
...
function MoveAny:InitArenaPrepFrames()
	if MoveAny:IsEnabled("ARENAPREPFRAMES", false) and Arena_LoadUI then
		...
		MAArenaPrepFrames = CreateFrame("FRAME", "MAArenaPrepFrames", MABack)
		MAArenaPrepFrames:SetSize(ArenaPrepFrames:GetSize())
		MAArenaPrepFrames:SetPoint("TOPRIGHT", MABack, "TOPRIGHT", 100, 100)
		...
		hooksecurefunc(MAArenaPrepFrames, "SetAlpha", function(sel, alpha)
			ArenaPrepFrames:SetAlpha(alpha)
		end)

		if MoveAny:DEBUG() then
			hooksecurefunc(ArenaPrepFrames, "Hide", function(sel)
				if sel.mahide then return end
				sel.mahide = true
				sel:Show()
				sel.mahide = false
